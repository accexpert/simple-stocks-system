...
public class TradeService extends BaseService implements IEventObserver {
    ...
    private IDataRepository dataTradeRepository;
    private IDataRepository dataStockRepository;
    private TradeCalculationsHandler tradeCalculationsHandler;

    public TradeService(IWriterHandler writerHandler, IEventHandler eventHandler, IDataRepository dataTradeRepository, IDataRepository dataStockRepository, TradeCalculationsHandler tradeCalculationsHandler) {
        super(writerHandler, eventHandler);
        this.dataTradeRepository = dataTradeRepository;
        this.dataStockRepository = dataStockRepository;
    ...
    public void notifyEvent(MessageEventModel event) {
        ...
        if(EventTypes.CALCULATE_STOCK.equals(event.getEventType())) {
            try {
                ...
                StockModel stockModel = (StockModel) dataStockRepository.getRecord(stockSymbol.name());
                TradeModel tradeModel = (TradeModel) dataTradeRepository.getLastRecord();
                ...
                Double geometricMean = tradeCalculationsHandler.calculateGeometricMean(dataTradeRepository.getRecords());
                Double stockPrice = tradeCalculationsHandler.calculateStockPrice(dataTradeRepository.getRecords(), (1000 * 60 * 15));
        ...
        if(EventTypes.RENDER_STOCK_DATA.equals(event.getEventType())) {
            ...
            try {
                if(null != stockSymbol) {
                    ...
                    stockSymbolsList.addAll(((List<StockModel>)dataStockRepository.getRecords()).parallelStream().map(val->val.getStockSymbol()).collect(Collectors.toList()));
                }
                stockSymbolsList.stream().forEach(val -> {
                    try {
                        StockModel stockModel = (StockModel) dataStockRepository.getRecord(val);
    ...
    public void run() {
        while(!getStop()) {
            try {
                StockModel stockModel = (StockModel)dataStockRepository.getRecord(getRandomStockSymbolValue().name());
                dataTradeRepository.addRecord(new TradeModel(stockModel.getStockId(), generateShareQuantity(stockModel.getShareQuantity()), generateTradeTypeRandom(), generatePrice()));
                Thread.sleep(1000);
    ...
    private void generateDummyDataStockRepository() throws SQLException {
        this.dataStockRepository.addRecord(new StockModel(StockSymbols.TEA.name(), StockTypes.COMMON, 0.0, 0.0, 100.0, 75));
        this.dataStockRepository.addRecord(new StockModel(StockSymbols.POP.name(), StockTypes.COMMON, 8.0, 0.0, 100.0, 80));
        this.dataStockRepository.addRecord(new StockModel(StockSymbols.ALE.name(), StockTypes.COMMON, 23.0, 0.0, 60.0, 70));
        this.dataStockRepository.addRecord(new StockModel(StockSymbols.GIN.name(), StockTypes.PREFERRED, 8.0, 2.0, 100.0, 85));
        this.dataStockRepository.addRecord(new StockModel(StockSymbols.JOE.name(), StockTypes.COMMON, 13.0, 0.0, 250.0, 95));
    }
}
